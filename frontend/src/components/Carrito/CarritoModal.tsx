...
export default function CarritoModal({
  ...
  // Compute image source robustly: cart items may use `imagen`, products use `imagenes` array
  ...
  const handleDeshacer = () => {
    ...
    showToast('Producto restaurado en el carrito', 'success');
    onClose();
  };
        ...
        <div className={styles['modal-content-wrapper']}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 36, marginBottom: 8 }}>
              {tipo === 'agregado' ? (
                <i className="fas fa-shopping-cart" style={{ color: '#002B5C', fontSize: 36 }} />
              ) : (
                <i className="fas fa-trash" style={{ color: '#C8102E', fontSize: 36 }} />
              )}
            </div>
            <h3 style={{ color: '#002B5C', marginBottom: 12 }}>{tipo === 'agregado' ? '¡Producto agregado al carrito!' : 'Producto removido del carrito'}</h3>
            ...
            <div style={{ background: '#f1f7fb', padding: 12, borderRadius: 8, display: 'flex', alignItems: 'center', gap: 12, maxWidth: 600, margin: '0 auto 16px' }}>
              ...
              <div style={{ textAlign: 'left' }}>
                ...
                <div style={{ color: '#0A7A5A', fontWeight: 700 }}>${(producto.precio ?? 0).toLocaleString()} x {cantidad}</div>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
              {tipo === 'agregado' ? (
                <>
                  ...
                  <button className="btn btn-primary" onClick={handleVerCarrito}><i className="fas fa-shopping-cart me-2" />Ver Carrito</button>
                </>
              ) : (
                <>
